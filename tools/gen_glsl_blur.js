...
console.log(`const lowp float step = ${step.toFixed(fixed)};`);
...
console.log(`	lowp float x = step;`);
for (var i = 1, x = step; i < N; i++) {
	console.log(`	o += tex(coord, size * x) * ${gk(x).toFixed(fixed)}; x += step;`);
	x += step;
}
