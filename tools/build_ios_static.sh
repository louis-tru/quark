...
CWD=`pwd`
PRODUCT_DIR="out/product/qgr/product"
BUILD_ALL=$1
BUILD_V8=$2
...
build() {
	...
	rm -f libqgr.a libv8.a
	
	ar rc libqgr.a `find obj.target/qgr-gui \
...
LIBS_NODEUI="out/ios.armv7.Release/libqgr.a 
							out/ios.arm64.Release/libqgr.a 
							out/ios.x64.Release/libqgr.a"
...
if [ "$BUILD_ALL" = 1 ]; then
	...
	LIBS_NODEUI="$OUT_LIBS_NODEUI
								out/ios.armv7s.Release/libqgr.a 
								out/ios.x86.Release/libqgr.a"
	LIBS_V8="$LIBS_V8 
...
lipo $LIBS_NODEUI -create -output ${PRODUCT_DIR}/ios/libqgr.a
if [ "$BUILD_V8" = 1 ]; then
