...
在上一篇中我已经为大家讲了[`Shark入门`](http://www.jianshu.com/p/b21bf5380c7f)，今天我就来介绍`shark`的核心部分（视图与布局）。

[View]为`gui`核心部件派生为[Notification]。
用它来描述屏幕上所有可见的元素,它是所有视图的基础类型它也是事件的响应者，这些事件由硬件以及操作系统触发。详细的API文档讲大家去[`这里`](http://shark1.org/doc/shark.html#class-view)查阅。

下面是`shark`现在提供的所有[View]继承关系图：
...
注意：
频繁的交替设置`transform`与调用`matrix`/`finalMatrix`会带来不必要的性能消耗，`shark`的渲染逻辑是在渲染画面前不对任何视图属性设置做额外的计算只做存储并该标记属性的改变，等待准备渲染前才做统一的计算。当一个视图的`transform`改变时如果这时你要获取`matrix`与`finalMatrix`那么直接返回`matrix`或`finalMatrix`那一定是不正确的，幸好系统会做检测当发生了改变你要强取些值会提前对这些值做运算，返回一个正确的值给你，但如果你频繁的设置与获取，那就会频繁的做些运算。并且这只仅限于非布局视图，在布局视图上这样做并不会返回正确的值参见[Trap in Layout]
...
从API文档上看见[Div]只有一个属性`contentAlign`, 那么这里重点讲述就是这个属性，因为它是与浏览器完全不相同有的地方，至于基础类型[Box]大家可以参与API文档会有详细说明，注意`padding`这个属性在`shark`里是没有的，因为这个导致系统复杂性上升，但以后要不要加待定。

在`shark`中[Div]并没有自己单独的浮动方式这个属性。但[Div]能设置它的`contentAlign`对它的内容对齐方式做出更改，这个属性可选的值有4个，默认为`left`左对齐
...
* [Span]并没有显示实体，也就是说它并不会在屏幕上显示任何可见的东西，它的存在只为是为了设置嵌套的[TextNode]视图属性，因为以[TextLayout]上的属性都能被它的子[TextLayout]所继承，这也是`shark`中唯具有继承性质属性的视图。
...
[Notification]: http://shark1.org/doc/event.html#class-notification
[View]: http://shark1.org/doc/shark.html#class-view
[TextFont]: http://shark1.org/doc/shark.html#class-textfont
[TextLayout]: http://shark1.org/doc/shark.html#class-textlayout
[View]: http://shark1.org/doc/shark.html#class-view
[Sprite]: http://shark1.org/doc/shark.html#class-sprite
[Layout]: http://shark1.org/doc/shark.html#class-layout
[Span]: http://shark1.org/doc/shark.html#class-span
[Box]: http://shark1.org/doc/shark.html#class-box
[Div]: http://shark1.org/doc/shark.html#class-div
[Hybrid]:  http://shark1.org/doc/shark.html#class-hybrid
[Limit]:  http://shark1.org/doc/shark.html#class-limit
[Indep]:  http://shark1.org/doc/shark.html#class-indep
[LimitIndep]:  http://shark1.org/doc/shark.html#class-limitindep
[Image]:  http://shark1.org/doc/shark.html#class-image
[Panel]:  http://shark1.org/doc/shark.html#class-panel
[Root]:  http://shark1.org/doc/shark.html#class-root
[BasicScroll]: http://shark1.org/doc/shark.html#class-basicscroll
[Scroll]: http://shark1.org/doc/shark.html#class-scroll
[Button]: http://shark1.org/doc/shark.html#class-button
[Text]: http://shark1.org/doc/shark.html#class-text
[Input]: http://shark1.org/doc/shark.html#class-input
[Textarea]: http://shark1.org/doc/shark.html#class-textarea
[TextNode]: http://shark1.org/doc/shark.html#class-textnode
[Label]: http://shark1.org/doc/shark.html#class-label
[Video]: http://shark1.org/doc/media.html#class-video
[Shadow]: http://shark1.org/doc/shark.html#class-shadow
[Trap in Layout]: http://shark1.org/doc/shark.html#trap-in-layout
