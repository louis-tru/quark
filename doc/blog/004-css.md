...
CSS样式表全称叫`Cascading Style Sheets`是一种用来表现HTML文件样式的语言，是Web前端开发中一定会用到的排版语言，那么Noug中css的灵感就来自于此。与其说是灵感还不如果说是借鉴并通过精简而来，因为开发这个框架的初衷效率一直就是最重的目标，其次才是使用体验。

下面是Noug中CSS样式表的写法：
```jsx
import { CSS, Div, Text } from 'noug';
CSS({
	'.a': {
		width: '100%',
		height: '100%',
...
并且越长的名称组合就会有越多的组合结果，也就是说查询也会需要更多的时间。比如`class="a b c"`的样式组合会有`.a`、`.b`、`.c`、`.a.b`、`.a.c`、`.b.c`、`.a.b.c` 7种结果，当视图应用这个样式时需要查询这7种可能性。所以在Noug中CSS样式表的组合限制在4个，多于4个时的组合时可能会出现意想不到结果。
...
[View.class]: https://noug.cc/doc/noug.html#get-view-class
[Action]: https://noug.cc/doc/action.html#class-action
[Div]: https://noug.cc/doc/noug.html#class-div
[Text]: https://noug.cc/doc/noug.html#class-text
