...
void main() {
  ...
  gl_Position = root_matrix * view_matrix * vec4(v.xy, 0., 1.);
}
...
void main(void) {
  float y = texture2D(s_tex_y,  f_tex_coord).r;
  float u = texture2D(s_tex_uv, f_tex_coord).r;
  float v = texture2D(s_tex_uv, f_tex_coord).a;
