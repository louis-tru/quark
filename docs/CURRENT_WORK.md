...
- New finding for problem 2: overlapping subpaths still need a merged boundary before AA generation. Tessellated body geometry already resolves overlapping filled regions, but raw `AASide` generation still sees the original subpath edges, so internal overlap edges can darken alpha and can flip the apparent inside/outside direction near intersections. A likely next step is an internal boundary extraction stage before `getAASideStrokeTriangle()` proper: original path -> merged boundary path -> inner body + AA band. libtess2 `TESS_BOUNDARY_CONTOURS` is a practical reference/implementation candidate; a full in-house solution would require polygon boolean clipping / planar graph face classification.
