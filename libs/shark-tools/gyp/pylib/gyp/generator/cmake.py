...
def WriteRules(target_name, rules, extra_sources, extra_deps,
  ...
  for rule in rules:
    ...
    SetVariableList(output, inputs_name,
        [NormjoinPathForceCMakeSource(path_to_gyp, dep) for dep in inputs])
    outputs = rule['outputs'] if 'outputs' in rule else []
    var_outputs = []
...
def WriteTarget(namer, qualified_target, target_dicts, build_dir, config_to_use,
  ...
  if cmake_target_type.command == 'add_library' or \
     cmake_target_type.command == 'add_executable':
    if not s_sources and not c_sources and not cxx_sources and not other_sources:
      path = os.path.abspath(os.path.dirname(__file__) + '/../../../data/placeholder.h')
    ...
    if params['parallel']:
      try:
        ...
        for config_name in config_names:
          ...
          # CallGenerateOutputForConfig(arglists[0])
