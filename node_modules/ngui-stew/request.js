...
class Request {
	...
	constructor(serverURL, mock, mockSwitch) {
		...
		this.m_enable_strict_response_date = false;
	}
	...
	get enableStrictResponseDate() { return this.m_enable_strict_response_date }
	set enableStrictResponseDate(value) { this.m_enable_strict_response_date = value }

	parseResponseData(buf) {
		var json = buf.toString('utf8');
		var res = JSON.parse(json);
		if (this.m_enable_strict_response_date) {
			if (res.code === 0) {
				return res.data;
			} else {
...
module.exports = {
	...
	get: async function(name, params, headers) {
		return await shared.request(name, 'GET', params, headers)
	},
	post: async function(name, params, headers) {
		return await shared.request(name, 'POST', params, headers)
	}, 
};
