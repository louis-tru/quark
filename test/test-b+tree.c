...
static inline void list_init(list_head_t *link)
{
	link->prev = link;
	link->next = link;
}
...
static inline void
__list_add(list_head_t *link, list_head_t *prev, list_head_t *next)
{
		link->next = next;
		link->prev = prev;
		next->prev = link;
...
static inline void list_add(list_head_t *link, list_head_t *prev)
{
		__list_add(link, prev, prev->next);
}

static inline void list_add_tail(list_head_t *link, list_head_t *head)
{
	__list_add(link, head->prev, head);
}

static inline void list_del(list_head_t *link)
{
...
static inline int is_leaf(bplus_node_t *node)
{
	return node->type == BPLUS_TREE_LEAF;
}

static int key_binary_search(bplus_node_t *node, key_t target)
{
	key_t *arr = key(node);
...
static inline int parent_key_index(bplus_node_t *parent, key_t key)
{
	int index = key_binary_search(parent, key);
	return index >= 0 ? index : -index - 2;
}

static inline bplus_node_t *cache_refer(bplus_tree_t *tree)
{
	int i;
	for (i = 0; i < MIN_CACHE_NUM; i++) {
		if (!tree->used[i]) {
			...
			return (bplus_node_t *) buf;
		}
	}
	assert(0);
}

static inline void cache_defer(bplus_tree_t *tree, bplus_node_t *node)
{
...
static bplus_node_t *node_new(bplus_tree_t *tree)
{
	bplus_node_t *node = cache_refer(tree);
...
static inline bplus_node_t *non_leaf_new(bplus_tree_t *tree)
{
	bplus_node_t *node = node_new(tree);
	node->type = BPLUS_TREE_NON_LEAF;
	return node;
}

static inline bplus_node_t *leaf_new(bplus_tree_t *tree)
{
	bplus_node_t *node = node_new(tree);
...
static bplus_node_t *node_fetch(bplus_tree_t *tree, off_t offset)
{
...
static bplus_node_t *node_seek(bplus_tree_t *tree, off_t offset)
{
	if (offset == INVALID_OFFSET) {
		return NULL;
	}
...
static inline void node_flush(bplus_tree_t *tree, bplus_node_t *node)
{
	if (node != NULL) {
...
static off_t new_node_append(bplus_tree_t *tree, bplus_node_t *node)
{
	...
	if (list_empty(&tree->free_blocks)) {
		...
		free_block_t *block;
		block = list_first_entry(&tree->free_blocks, free_block_t, link);
		list_del(&block->link);
...
static void node_delete(bplus_tree_t *tree, bplus_node_t *node,
			bplus_node_t *left, bplus_node_t *right)
{
	if (left != NULL) {
		if (right != NULL) {
	...
	free_block_t *block = malloc(sizeof(*block));
	assert(block != NULL);
...
static inline void sub_node_update(bplus_tree_t *tree, bplus_node_t *parent,
				   int index, bplus_node_t *sub_node)
{
...
static inline void sub_node_flush(bplus_tree_t *tree, bplus_node_t *parent, off_t sub_offset)
{
	bplus_node_t *sub_node = node_fetch(tree, sub_offset);
	assert(sub_node != NULL);
	sub_node->parent = parent->self;
	node_flush(tree, sub_node);
}

static long bplus_tree_search(bplus_tree_t *tree, key_t key)
{
	int ret = -1;
	bplus_node_t *node = node_seek(tree, tree->root);
	while (node != NULL) {
...
static void left_node_add(bplus_tree_t *tree, bplus_node_t *node, bplus_node_t *left)
{
	new_node_append(tree, left);

	bplus_node_t *prev = node_fetch(tree, node->prev);
	if (prev != NULL) {
		prev->next = left->self;
		left->prev = prev->self;
...
static void right_node_add(bplus_tree_t *tree, bplus_node_t *node, bplus_node_t *right)
{
	new_node_append(tree, right);

	bplus_node_t *next = node_fetch(tree, node->next);
	if (next != NULL) {
...
static key_t non_leaf_insert(bplus_tree_t *tree, bplus_node_t *node,
			     bplus_node_t *l_ch, bplus_node_t *r_ch, key_t key);

static int parent_node_build(bplus_tree_t *tree, bplus_node_t *l_ch,
			     bplus_node_t *r_ch, key_t key)
{
	if (l_ch->parent == INVALID_OFFSET && r_ch->parent == INVALID_OFFSET) {
		/* new parent */
		bplus_node_t *parent = non_leaf_new(tree);
		key(parent)[0] = key;
...
static key_t non_leaf_split_left(bplus_tree_t *tree, bplus_node_t *node,
				 bplus_node_t *left, bplus_node_t *l_ch,
				 bplus_node_t *r_ch, key_t key, int insert)
{
	int i;
	key_t split_key;
...
static key_t non_leaf_split_right1(bplus_tree_t *tree, bplus_node_t *node,
				   bplus_node_t *right, bplus_node_t *l_ch,
				   bplus_node_t *r_ch, key_t key, int insert)
{
	int i;
...
static key_t non_leaf_split_right2(bplus_tree_t *tree, bplus_node_t *node,
				   bplus_node_t *right, bplus_node_t *l_ch,
				   bplus_node_t *r_ch, key_t key, int insert)
{
	int i;
...
static void non_leaf_simple_insert(bplus_tree_t *tree, bplus_node_t *node,
				   bplus_node_t *l_ch, bplus_node_t *r_ch,
...
static int non_leaf_insert(bplus_tree_t *tree, bplus_node_t *node,
			   bplus_node_t *l_ch, bplus_node_t *r_ch, key_t key)
{
	/* Search key location */
	...
	if (node->children == _max_order) {
		...
		bplus_node_t *sibling = non_leaf_new(tree);
		if (insert < split) {
...
static key_t leaf_split_left(bplus_tree_t *tree, bplus_node_t *leaf,
			     bplus_node_t *left, key_t key, long data, int insert)
{
	/* split = [m/2] */
...
static key_t leaf_split_right(bplus_tree_t *tree, bplus_node_t *leaf,
			      bplus_node_t *right, key_t key, long data, int insert)
{
	/* split = [m/2] */
	int split = (leaf->children + 1) / 2;
...
static void leaf_simple_insert(bplus_tree_t *tree, bplus_node_t *leaf,
...
static int leaf_insert(bplus_tree_t *tree, bplus_node_t *leaf, key_t key, long data)
{
	/* Search key location */
	...
	if (leaf->children == _max_entries) {
		...
		bplus_node_t *sibling = leaf_new(tree);
...
static int bplus_tree_insert(bplus_tree_t *tree, key_t key, long data)
{
	bplus_node_t *node = node_seek(tree, tree->root);
	while (node != NULL) {
		if (is_leaf(node)) {
	...
	/* new root */
	bplus_node_t *root = leaf_new(tree);
	key(root)[0] = key;
	data(root)[0] = data;
	root->children = 1;
...
static inline int sibling_select(bplus_node_t *l_sib, bplus_node_t *r_sib,
				 bplus_node_t *parent, int i)
{
	if (i == -1) {
...
static void non_leaf_shift_from_left(bplus_tree_t *tree, bplus_node_t *node,
				     bplus_node_t *left, bplus_node_t *parent,
...
static void non_leaf_merge_into_left(bplus_tree_t *tree, bplus_node_t *node,
				     bplus_node_t *left, bplus_node_t *parent,
				     int parent_key_index, int remove)
{
...
static void non_leaf_shift_from_right(bplus_tree_t *tree, bplus_node_t *node,
				      bplus_node_t *right, bplus_node_t *parent,
...
static void non_leaf_merge_from_right(bplus_tree_t *tree, bplus_node_t *node,
				      bplus_node_t *right, bplus_node_t *parent,
				      int parent_key_index)
{
	/* move parent key down */
...
static inline void non_leaf_simple_remove(bplus_tree_t *tree, bplus_node_t *node, int remove)
{
...
static void non_leaf_remove(bplus_tree_t *tree, bplus_node_t *node, int remove)
{
	if (node->parent == INVALID_OFFSET) {
		/* node is the root */
		if (node->children == 2) {
			/* replace old root with the first sub-node */
			bplus_node_t *root = node_fetch(tree, sub(node)[0]);
		...
		bplus_node_t *l_sib = node_fetch(tree, node->prev);
		bplus_node_t *r_sib = node_fetch(tree, node->next);
		bplus_node_t *parent = node_fetch(tree, node->parent);
		...
		if (sibling_select(l_sib, r_sib, parent, i)  == LEFT_SIBLING) {
			...
			if (r_sib->children > (_max_order + 1) / 2) {
				...
				bplus_node_t *rr_sib = node_fetch(tree, r_sib->next);
...
static void leaf_shift_from_left(bplus_tree_t *tree, bplus_node_t *leaf,
				 bplus_node_t *left, bplus_node_t *parent,
				 int parent_key_index, int remove)
{
...
static void leaf_merge_into_left(bplus_tree_t *tree, bplus_node_t *leaf,
				 bplus_node_t *left, int parent_key_index, int remove)
{
...
static void leaf_shift_from_right(bplus_tree_t *tree, bplus_node_t *leaf,
				  bplus_node_t *right, bplus_node_t *parent,
				  int parent_key_index)
{
...
static inline void leaf_merge_from_right(bplus_tree_t *tree, bplus_node_t *leaf,
					 bplus_node_t *right)
{
...
static inline void leaf_simple_remove(bplus_tree_t *tree, bplus_node_t *leaf, int remove)
{
...
static int leaf_remove(bplus_tree_t *tree, bplus_node_t *leaf, key_t key)
{
	...
	if (leaf->parent == INVALID_OFFSET) {
		...
		bplus_node_t *l_sib = node_fetch(tree, leaf->prev);
		bplus_node_t *r_sib = node_fetch(tree, leaf->next);
		bplus_node_t *parent = node_fetch(tree, leaf->parent);
...
static int bplus_tree_delete(bplus_tree_t *tree, key_t key)
{
	bplus_node_t *node = node_seek(tree, tree->root);
	while (node != NULL) {
		if (is_leaf(node)) {
...
bplus_tree_t *bplus_tree_init(char *filename, int block_size)
{
	...
	bplus_tree_t *tree = calloc(1, sizeof(*tree));
	assert(tree != NULL);
...
void bplus_tree_deinit(bplus_tree_t *tree)
{
	...
	list_for_each_safe(pos, n, &tree->free_blocks) {
		list_del(pos);
		free_block_t *block = list_entry(pos, free_block_t, link);
...
static inline int children(bplus_node_t *node)
{
	assert(!is_leaf(node));
	return node->children;
}

static void node_key_dump(bplus_node_t *node)
{
	int i;
	if (is_leaf(node)) {
		printf("leaf:");
...
static void draw(bplus_tree_t *tree, bplus_node_t *node, struct node_backlog *stack, int level)
{
	int i;
	for (i = 1; i < level; i++) {
		if (i == level - 1) {
