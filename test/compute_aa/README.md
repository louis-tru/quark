...
| `experiment/compute-aa-cpu-backdrop` | `8e8e2682a` | CPU 二维差分与前缀和生成完整 backdrop；每 tile 16线程，两个 tile 配成 SIMD32；每线程负责一行像素；无共享 mask/barrier | 约 `0.60ms` | 历史 CPU-backdrop 对照 |
...
| `experiment/compute-aa-sorted-crossings` | 当前分支 | 在边界/纯色分流上使用稀疏 X bucket、`crossingMask + ctz()` 与区间 mask | 总时间约 `0.32-0.40ms`；Boundary 约 `9%` | 当前最快，正式接入基线 |
...
- 当前稀疏交点分桶分支是实测最快版本，也是正式接入基线；
- boundary/uniform 分流是总时间从约 `0.60-0.77ms` 继续降至
  `0.32-0.40ms` 的关键，CPU 完整 backdrop 分支已不再是最快方案；
- 64线程/shared-mask 与 16线程/GPU-event 两个旧版本基本持平，说明只改变
  线程组织、shared mask 和 barrier 不能突破旧瓶颈；
- 全共享 `windingDelta[64][64]` 路线已经确认不通过；
- 当前剩余主要瓶颈是 Composite，而不是 CPU 数据构建或 Boundary Coverage。
